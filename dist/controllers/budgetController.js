...
exports.default = {
    ...
    getByVehicleID: async (request, response) => {
        try {
            const { id } = request.params;
            if (!id)
                throw new Error('ID is required');
            const budget = await db_1.default.budget.findFirstOrThrow({
                where: {
    ...
    getByID: async (request, response) => {
        try {
            ...
            if (!id)
                throw new Error('ID is required');
            const budget = await db_1.default.budget.findUniqueOrThrow({
                where: {
                    id: parseInt(id)
                },
                select: {
    ...
    toReceive: async (request, response) => {
        try {
            const { id } = request.params;
            if (!id)
                throw new Error('ID is required');
